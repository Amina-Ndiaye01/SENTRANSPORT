...
function App() {
  ...
  return (
    <div className="App">
      <Header />
      <p style={{ textAlign: 'center', color: '#7f8c8d', fontSize: '0.9rem', margin: '10px 0' }}>
      ...
      <main className="contenu">
        <div style={{ display: 'flex', gap: '10px', alignItems: 'center', marginBottom: '20px' }}>
          ...
          {recherche && (
            <button onClick={() => setRecherche("")} style={{ padding: '10px 16px', backgroundColor: '#e74c3c', color: 'white', border: 'none', borderRadius: '8px', cursor: 'pointer', whiteSpace: 'nowrap', flexShrink: 0 }}>
              ✕ Effacer
            </button>
          )}
        </div>
        ...
        {lignesFiltrees.length === 0 && (
          <p style={{ textAlign: 'center', color: '#e74c3c', fontSize: '1rem', marginTop: '30px' }}>
          {'Aucune ligne trouvée pour "'+ recherche + '"' }"
          </p>
        )}
        {lignesFiltrees.map(ligne => (
          <LigneBus key={ligne.id} numero={ligne.numero} depart={ligne.depart} arrivee={ligne.arrivee} arrets={ligne.arrets} estSelectionnee={ligneSelectionnee && ligneSelectionnee.id === ligne.id} onClick={() => handleClickLigne(ligne)} />
        ))}
